...
incomplete_path = "./review_polarity/txt_sentoken/"
...
test_set_path_classified = './test_set_classified.csv'
test_set_path_classified_ = './test_set_classified_.csv'
test_set_path_classified__ = './test_set_classified__.csv'
...
def write_training_set():
    with open(training_set_path, 'w') as csvfile:
...
def read_training_set():
    with open(training_set_path, 'r') as csvfile:
...
data_frame_path = './data_frame__.csv'
removed_words_path = './removed_words__.csv'
...
# write test_set
def write_test_set():
    with open(test_set_path, 'w') as csvfile:
        writer = csv.writer(csvfile)
        for row in test_set:
            writer.writerow(row)
...
def write_test_set_classified():
    with open(test_set_path_classified__, 'w') as csvfile:
        writer = csv.writer(csvfile)
        for row in test_set:
            writer.writerow(row)
...
def naive_bayes_classify(document, labels, processed_words, class_probabilities, words_per_class, df):
    ...
    for label in labels:
        print (math.log(class_probabilities[label], 2), no_words_in_doc, math.log(words_per_class[label], 2))
